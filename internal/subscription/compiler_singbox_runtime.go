...
func singBoxDNS(custom map[string]any, version string, shared dnsShared, remoteOutbound string, bootstrapDomains []string) map[string]any {
	modern := version != "11"
	if override := singBoxDNSOverride(custom, modern); override != nil {
		return override
	}
	localDNS, localSystem := localDNSServer(shared.LocalDNS, shared.LocalDNSTransport)
	if modern {
		local := modernLocalDNSServer("local", localDNS, shared)
		servers := []any{local}
		...
		if !localSystem {
			servers = append(servers, modernLocalDNSServer("local_v4", localDNS, shared))
		}
		...
		if len(bootstrapDomains) > 0 {
			rules = append([]any{map[string]any{"action": "route", "domain": bootstrapDomains, "server": "bootstrap"}}, rules...)
		}
	...
	servers := []any{map[string]any{"tag": "local", "address": legacyLocalDNSAddress(localDNS, shared)}}
	if shared.FakeIPEnabled {
		servers = append(servers, map[string]any{"tag": "fakeip", "address": "fakeip", "strategy": "ipv4_only"})
	}
	if !localSystem {
		servers = append(servers, map[string]any{"tag": "local_v4", "address": legacyLocalDNSAddress(localDNS, shared), "strategy": "ipv4_only"})
	}
	...
	rules := singBoxDNSRules(shared, false, false, shared.FakeIPEnabled)
	if len(bootstrapDomains) > 0 {
		rules = append([]any{map[string]any{"domain": bootstrapDomains, "server": "bootstrap"}}, rules...)
	}
	result := map[string]any{"disable_cache": false, "servers": servers, "rules": rules, "disable_expire": false, "independent_cache": false, "reverse_mapping": true, "final": "remote"}
	if shared.PreferIPv4 {
		result["strategy"] = "prefer_ipv4"
	}
	if shared.FakeIPEnabled {
...
func localDNSServer(value, transport string) (string, bool) {
	for part := range strings.SplitSeq(value, ",") {
		part = strings.TrimSpace(part)
		if part == "" {
			continue
		}
		return part, transport == "system" || transport == "" && strings.EqualFold(part, "local")
	}
	return "local", true
}

func modernLocalDNSServer(tag, server string, shared dnsShared) map[string]any {
	if _, system := localDNSServer(server, shared.LocalDNSTransport); system {
		return map[string]any{"type": "local", "tag": tag}
	}
	result := map[string]any{"type": valueOr(shared.LocalDNSTransport, "udp"), "tag": tag, "server": server, "server_port": shared.LocalDNSPort}
	if shared.LocalDNSTransport == "tls" && shared.LocalServerName != "" {
		result["tls"] = map[string]any{"server_name": shared.LocalServerName}
	}
	return result
}

func legacyLocalDNSAddress(server string, shared dnsShared) string {
	if shared.LocalDNSTransport == "tls" {
		return fmt.Sprintf("tls://%s:%d", server, shared.LocalDNSPort)
	}
	return server
}

type dnsShared struct {
	LocalDNS, LocalDNSTransport, LocalServerName, FakeIPIPv4Range, FakeIPIPv6Range            string
	BootstrapDNS, BootstrapServerName, RemoteDNS, RemoteServerName, RemoteDetour              string
	CNDomainRuleSetURL, CNDomainRuleSetDetour, CNIPRuleSetURL, CNIPRuleSetDetour              string
	HKIPRuleSetURL, HKIPRuleSetDetour                                                         string
	SystemDNSListenHosts                                                                      []string
	LocalDNSPort, FakeIPTTL                                                                   int
	BootstrapDNSPort, RemoteDNSPort                                                           int
	SystemDNSListenPort                                                                       int
	FakeIPEnabled, RejectHTTPS, CNDomainLocalDNS, CNIPLocalDNS, ExcludeHKFromCNIP, PreferIPv4 bool
	CNDomainRuleSetEnabled, CNIPRuleSetEnabled, HKIPRuleSetEnabled                            bool
	SystemDNSTakeoverEnabled                                                                  bool
}

func resolveDNSShared(config map[string]any) dnsShared {
	result := dnsShared{LocalDNS: "223.5.5.5", LocalDNSTransport: "udp", LocalDNSPort: 53, FakeIPIPv4Range: "198.18.0.0/15", FakeIPIPv6Range: "fc00::/18", FakeIPEnabled: true, FakeIPTTL: 300, RejectHTTPS: true, CNDomainLocalDNS: true, CNIPLocalDNS: true, ExcludeHKFromCNIP: true, BootstrapDNS: "223.5.5.5", BootstrapDNSPort: 853, BootstrapServerName: "dns.alidns.com", RemoteDNS: "8.8.8.8", RemoteDNSPort: 853, RemoteServerName: "dns.google", PreferIPv4: true, CNDomainRuleSetEnabled: true, CNDomainRuleSetURL: "https://cdn.jsdelivr.net/gh/SagerNet/sing-geosite@rule-set/geosite-cn.srs", CNDomainRuleSetDetour: "direct", CNIPRuleSetEnabled: true, CNIPRuleSetURL: "https://cdn.jsdelivr.net/gh/SagerNet/sing-geoip@rule-set/geoip-cn.srs", CNIPRuleSetDetour: "direct", HKIPRuleSetEnabled: true, HKIPRuleSetURL: "https://cdn.jsdelivr.net/gh/SagerNet/sing-geoip@rule-set/geoip-hk.srs", HKIPRuleSetDetour: "direct", SystemDNSListenPort: 53, SystemDNSListenHosts: []string{"127.0.0.1"}}
	shared := config
	if nested, ok := objectValue(config["shared"]); ok {
		shared = nested
	}
	result.LocalDNS = valueOr(stringValue(shared["localDns"]), result.LocalDNS)
	if transport := stringValue(shared["localDnsTransport"]); transport != "" {
		result.LocalDNSTransport = transport
	} else if strings.EqualFold(result.LocalDNS, "local") {
		result.LocalDNSTransport = "system"
	}
	result.LocalServerName = stringValue(shared["localServerName"])
	...
	result.CNIPLocalDNS = boolDefault(shared["cnIpLocalDns"], result.CNIPLocalDNS)
	result.ExcludeHKFromCNIP = boolDefault(shared["excludeHkFromCnIp"], result.ExcludeHKFromCNIP)
	result.CNDomainRuleSetEnabled = boolDefault(shared["cnDomainRuleSetEnabled"], result.CNDomainRuleSetEnabled)
	result.CNDomainRuleSetURL = valueOr(stringValue(shared["cnDomainRuleSetUrl"]), result.CNDomainRuleSetURL)
	result.CNDomainRuleSetDetour = valueOr(stringValue(shared["cnDomainRuleSetDetour"]), result.CNDomainRuleSetDetour)
	result.CNIPRuleSetEnabled = boolDefault(shared["cnIpRuleSetEnabled"], result.CNIPRuleSetEnabled)
	result.CNIPRuleSetURL = valueOr(stringValue(shared["cnIpRuleSetUrl"]), result.CNIPRuleSetURL)
	result.CNIPRuleSetDetour = valueOr(stringValue(shared["cnIpRuleSetDetour"]), result.CNIPRuleSetDetour)
	result.HKIPRuleSetEnabled = boolDefault(shared["hkIpRuleSetEnabled"], result.HKIPRuleSetEnabled)
	result.HKIPRuleSetURL = valueOr(stringValue(shared["hkIpRuleSetUrl"]), result.HKIPRuleSetURL)
	result.HKIPRuleSetDetour = valueOr(stringValue(shared["hkIpRuleSetDetour"]), result.HKIPRuleSetDetour)
...
func validateSingBoxSystemDNS(shared dnsShared, target Target, profile Profile) error {
	if err := validateSingBoxDNSShared(shared); err != nil {
		return err
	}
	...
	if _, localSystem := localDNSServer(shared.LocalDNS, shared.LocalDNSTransport); localSystem {
...
func validateSingBoxDNSShared(shared dnsShared) error {
	switch shared.LocalDNSTransport {
	case "system", "udp", "tls":
	default:
		return fmt.Errorf("unsupported local DNS transport %q", shared.LocalDNSTransport)
	}
	if shared.LocalDNSTransport != "system" && strings.TrimSpace(shared.LocalDNS) == "" {
		return fmt.Errorf("local DNS upstream is required")
	}
	for _, item := range []struct {
		...
		{shared.CNDomainRuleSetEnabled, "CN domain rule-set", shared.CNDomainRuleSetURL},
		{shared.CNIPRuleSetEnabled, "CN IP rule-set", shared.CNIPRuleSetURL},
		{shared.HKIPRuleSetEnabled, "HK IP rule-set", shared.HKIPRuleSetURL},
	} {
		if item.enabled && strings.TrimSpace(item.url) == "" {
			return fmt.Errorf("%s URL is required when enabled", item.name)
		}
	}
	return nil
}
...
func singBoxDNSRules(shared dnsShared, modern, responseMatching, fakeIP bool) []any {
	...
	if shared.CNDomainLocalDNS && shared.CNDomainRuleSetEnabled {
	...
	if shared.CNIPLocalDNS && shared.CNIPRuleSetEnabled {
		cnIPRule := map[string]any{"rule_set": []string{"geoip-cn"}, "server": "local"}
		if shared.ExcludeHKFromCNIP && shared.HKIPRuleSetEnabled {
			cnIPRule = map[string]any{
				"type": "logical", "mode": "and", "server": "local",
				"rules": []any{
					map[string]any{"rule_set": []string{"geoip-cn"}},
					map[string]any{"invert": true, "rule_set": []string{"geoip-hk"}},
				},
			}
		}
		if modern {
			cnIPRule["action"] = "route"
		}
		rules = append(rules, cnIPRule)
	}
