...
pub const DOMESTIC_IP_PROBE: PublicIpProbe = PublicIpProbe {
    id: "domestic-ip",
    name: "Domestic IP",
    region: "domestic",
...
pub const FOREIGN_IP_PROBE: PublicIpProbe = PublicIpProbe {
    id: "foreign-ip",
    name: "Foreign IP",
    region: "foreign",
...
pub async fn lookup_ip_metadata(client: &Client, ip: &str) -> Result<IpMetadata, String> {
...
mod tests {
    ...
    fn parses_text_and_json_ip_responses() {
        assert_eq!(
            DOMESTIC_IP_PROBE
                .parse_response(b"current address: 203.0.113.7\n")
                .expect("text IP"),
            "203.0.113.7"
        );
        assert_eq!(
            FOREIGN_IP_PROBE
                .parse_response(br#"{"ip":"2001:db8::1"}"#)
                .expect("JSON IP"),
            "2001:db8::1"
        );
