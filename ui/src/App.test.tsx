...
describe('App', () => {
  ...
  it('enters the lazy shell after login under StrictMode when the core is unavailable', async () => {
    ...
    const networkAutomation = screen.getByRole('link', { name: 'Automatic network switching' })
    fireEvent.click(networkAutomation)
    expect(await screen.findByRole('heading', { name: 'Automatic network switching' })).toBeInTheDocument()
