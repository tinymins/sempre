...
import { ServiceVersionReload } from './features/service/ServiceVersionReload'
...
const NetworkAutomation = lazy(() => import('./pages/NetworkAutomation').then((module) => ({ default: module.NetworkAutomation })))
...
export function AppContent({ serverMode = serverBuild }: { serverMode?: boolean }) {
  ...
  return <><ServiceVersionReload /><ServiceUpdateFlow>{!session && !isDevShowcase ? <Login /> : <Suspense fallback={<div className="grid min-h-screen place-items-center"><Spinner /></div>}><HashRouter><Shell><Routes><Route path="/" element={<Overview />} /><Route path="/custom-nodes" element={<CustomNodes />} /><Route path="/subscriptions" element={<Subscriptions />} /><Route path="/network-automation" element={<NetworkAutomation />} /><Route path="/tunnels" element={<Tunnels />} /><Route path="/proxies" element={<Proxies />} /><Route path="/connections" element={<Connections />} /><Route path="/routing-rules" element={<RoutingRules />} /><Route path="/rules" element={<Rules />} /><Route path="/dns" element={<Dns />} /><Route path="/traffic" element={<Traffic />} /><Route path="/logs" element={<Logs />} /><Route path="/network-test" element={<NetworkTest />} /><Route path="/node-test" element={<NodeTest />} /><Route path="/runtime-status" element={<RuntimeStatus />} /><Route path="/gateway" element={<GatewayRoute />} /><Route path="/management" element={<Management />} />{AcmeShowcase ? <Route path="/components" element={<AcmeShowcase />} /> : null}<Route path="*" element={<Navigate to="/" replace />} /></Routes></Shell></HashRouter></Suspense>}</ServiceUpdateFlow></>
}
