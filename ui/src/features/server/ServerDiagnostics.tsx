...
export function ServerDiagnostics({ session, profileId, sources, target, targets }: Props) {
  ...
  const clearCache = () => run('cache', async () => {
    ...
    setNotice('Source cache cleared.')
  })

  return <Card className="space-y-4 p-4">
    <div><h3 className="font-medium">Source and conversion diagnostics</h3><p className="text-xs text-[var(--muted)]">Test a saved source live, inspect the merged node list, and trace one node through filtering and target conversion.</p></div>
    {notice ? <p role={notice === 'Source cache cleared.' ? 'status' : 'alert'} className="text-sm">{notice}</p> : null}
    <div className="flex flex-wrap items-end gap-2">
      <Field label="Source"><Select className="min-w-56" value={effectiveSourceId} options={sources.map((source) => ({ value: source.id, label: source.remark || source.url || source.id }))} onChange={(value) => setSourceId(String(value))} /></Field>
      <Button disabled={!effectiveSourceId || Boolean(pending)} onClick={testSource}>{pending === 'source' ? <Spinner /> : null}Test source now</Button>
      <Button disabled={!effectiveSourceId || Boolean(pending)} onClick={clearCache}>{pending === 'cache' ? <Spinner /> : null}Clear source cache</Button>
    </div>
    ...
    <div className="flex flex-wrap items-end gap-2">
      <Button disabled={Boolean(pending)} onClick={preview}>{pending === 'preview' ? <Spinner /> : null}Preview merged nodes</Button>
      {nodes.length ? <Field label="Node"><Select className="min-w-56" value={nodeName} options={nodes.map((node) => ({ value: node.name, label: `${node.filtered ? 'filtered · ' : ''}${node.name}` }))} onChange={(value) => setNodeName(String(value))} /></Field> : null}
      {nodes.length ? <Button disabled={!nodeName || Boolean(pending)} onClick={traceNode}>{pending === 'trace' ? <Spinner /> : null}Trace selected node</Button> : null}
    </div>
