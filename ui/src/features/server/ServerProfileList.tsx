  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl space-y-6 p-5">
      <header className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button variant="ghost" onClick={onLogout}>{t('signOut')}</Button>
      </header>
      <Card className="p-5">
        <h2 className="font-semibold">{t('createProfile')}</h2>
        <div className="mt-3 flex items-end gap-2">
          <Field label={t('profileName')}><Input value={name} onChange={(event) => setName(event.target.value)} /></Field>
          <Button variant="primary" disabled={pending || !name.trim()} onClick={create}>{pending ? <Spinner /> : <Plus size={16} />}{t('create')}</Button>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {profiles.map((profile) => (
            <Card key={profile.id} className="cursor-pointer p-4 transition-colors hover:bg-[var(--surface-hover)]" role="button" tabIndex={0} onClick={() => navigate(`/server/subscriptions/${profile.id}`)} onKeyDown={(event) => { if (event.key === 'Enter') navigate(`/server/subscriptions/${profile.id}`) }}>
              ...
              <p className="mt-2 text-xs text-[var(--muted)]">{t('revision')} {profile.revision} · {t('updated')} {new Date(profile.updated_at).toLocaleString()}</p>
              <p className="mt-3 flex items-center gap-1 text-sm text-[var(--muted)]"><Users size={14} />{t('multiUserProfile')}</p>
            </Card>
          ))}
          {!profiles.length ? <Card className="p-8 text-center text-sm text-[var(--muted)]">{t('noProfiles')}</Card> : null}
        </div>
      )}
    </main>
  )
