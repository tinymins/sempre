...
import { Check, CheckCircle2, CircleAlert, Download, LoaderCircle, PackageCheck, RefreshCw, Upload } from 'lucide-react'
import { useI18n } from '../../lib/i18n'
import type { ServiceUpdateTask } from '../../lib/types'

const stages = ['prepare', 'download', 'verify', 'install', 'complete'] as const

export function ServiceUpdateModal({ open, task, targetVersion, submitting, uploading, disconnected, error, allowClose = true, onClose, onRelogin }: {
  open: boolean
  ...
  const stage: ServiceUpdateTask['stage'] | 'uploading' = uploading && submitting ? 'uploading' : disconnected && running ? 'installing' : task?.stage || 'checking'
  ...
  return <Modal open={open} centered width="min(760px, calc(100vw - 32px))" maskClosable={false} closable={!succeeded && allowClose} keyboard={!succeeded && allowClose} onCancel={onClose}
    ...
    footer={<div className="flex w-full items-center justify-between gap-4"><span className="text-xs text-[var(--muted)]">{succeeded ? (zh ? '更新已完成，请重新登录以使用新版本。' : 'The update is complete. Sign in again to use the new version.') : running ? (allowClose ? (zh ? '任务在后台执行，关闭窗口不会中止更新。' : 'The update continues in the background if this window is closed.') : (zh ? '正在等待更新完成，请稍候。' : 'Waiting for the update to finish.')) : (zh ? '更新未完成，请检查错误后重试。' : 'The update did not complete. Check the error before retrying.')}</span>{succeeded ? <Button variant="primary" onClick={onRelogin}>{zh ? '重新登录' : 'Sign in again'}</Button> : allowClose ? <Button onClick={onClose}>{zh ? '关闭' : 'Close'}</Button> : null}</div>}>
...
function CurrentStage({ task, stage, submitting, disconnected, zh }: { task?: ServiceUpdateTask | null; stage: string; submitting: boolean; disconnected: boolean; zh: boolean }) {
  const transferring = stage === 'downloading' || stage === 'uploading'
  const percent = task?.total_bytes ? Math.min(100, task.downloaded_bytes * 100 / task.total_bytes) : 0
  const icon = stage === 'uploading' ? <Upload size={24} /> : stage === 'downloading' ? <Download size={24} /> : stage === 'completed' ? <PackageCheck size={24} /> : disconnected ? <RefreshCw size={24} className="animate-spin" /> : <LoaderCircle size={24} className="animate-spin" />
  ...
  return <div role="status" className="rounded-lg border border-[var(--border)] p-5">
    ...
    {transferring && task ? <div className="mt-5 space-y-2"><div role="progressbar" aria-label={stage === 'uploading' ? (zh ? '更新包上传进度' : 'Update upload progress') : (zh ? '更新下载进度' : 'Update download progress')} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(percent)}><Progress percent={percent} status="active" /></div><div className="grid grid-cols-2 gap-3 text-xs text-[var(--muted)] sm:grid-cols-3"><Metric label={stage === 'uploading' ? (zh ? '上传进度' : 'Uploaded') : (zh ? '下载进度' : 'Downloaded')} value={`${formatBytes(task.downloaded_bytes)} / ${formatBytes(task.total_bytes)}`} /><Metric label={zh ? '实时速度' : 'Speed'} value={task.bytes_per_second ? `${formatBytes(task.bytes_per_second)}/s` : '—'} /><Metric label={zh ? '预计剩余' : 'ETA'} value={task.eta_seconds == null ? '—' : formatEta(task.eta_seconds, zh)} /></div>{task.artifact ? <p className="truncate font-mono text-[11px] text-[var(--muted)]">{task.artifact}</p> : null}</div> : null}
  </div>
}
...
function stepLabel(step: typeof stages[number], zh: boolean, uploading: boolean) {
  const labels = { prepare: ['准备', 'Prepare'], download: ['下载', 'Download'], verify: ['校验', 'Verify'], install: ['安装', 'Install'], complete: ['完成', 'Complete'] }
  if (step === 'download' && uploading) return zh ? '上传' : 'Upload'
  return labels[step][zh ? 0 : 1]
}

function stageCopy(stage: string, submitting: boolean, disconnected: boolean, zh: boolean) {
  if (submitting && stage === 'uploading') return zh ? ['正在上传更新包', '上传完成后将继续执行原有的解压、校验和安装流程。'] : ['Uploading update package', 'The existing extraction, validation, and installation flow will continue after upload.']
