...
function NodeEditor({ open, node, profiles, onClose, onSaved, onSettled, afterOpenChange }: { open: boolean; node?: CustomNode; profiles: SubscriptionProfile[]; onClose: () => void; onSaved: () => void; onSettled: () => Promise<unknown>; afterOpenChange: (open: boolean) => void }) {
  ...
  const [selectedIDs, setSelectedIDs] = useState(() => profiles.filter((profile) => node ? profile.custom_node_ids.includes(node.id) : profile.mode !== 'remote').map((profile) => profile.id))
  const [error, setError] = useState('')
  const save = useMutation({
    mutationFn: () => {
      const proxy = parseJSONC<Record<string, unknown>>(content)
      if (!proxy.name && name) proxy.name = name
      const body = { name: name || String(proxy.name || ''), proxy, subscription_ids: selectedIDs }
  ...
  return (
    ...
    >
      <div className="grid gap-4">
        ...
        <Form.Item label={t('subscriptionSets')} extra={t('nodeSubscriptionsHint')} className="mb-0">
          <Select mode="multiple" showSearch allowClear className="w-full" value={selectedIDs} onChange={setSelectedIDs} disabled={save.isPending} placeholder={t('selectSubscriptionSets')} options={profiles.map((profile) => ({ value: profile.id, label: `${profile.name || t('defaultSubscriptionSet')}${profile.mode === 'remote' ? ` (${t('readOnly')})` : ''}`, disabled: profile.mode === 'remote' }))} />
        </Form.Item>
