...
function Settings({ current, setDraft, status, zh }: { current: DnsSettings; setDraft: (value: DnsSettings) => void; status?: DnsFrontendStatus; zh: boolean }) {
  ...
  const port53 = status?.port_53
  return <div className="space-y-5 pt-4">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      <Metric label={zh ? '前置 DNS' : 'DNS frontend'} value={status?.running ? (zh ? '运行中' : 'Running') : status?.enabled ? (zh ? '待启动' : 'Pending') : (zh ? '未启用' : 'Disabled')} />
      <Metric label={zh ? '核心 DNS 模式' : 'Core DNS mode'} value={status?.mode || '-'} />
      <Metric label={zh ? '核心 DNS' : 'Core DNS'} value={status?.core_dns_healthy ? (zh ? '正常' : 'Healthy') : '-'} />
      {port53 ? <Metric label={zh ? 'DNS 53 端口' : 'DNS port 53'} value={port53.listening ? (zh ? '已接管' : 'Captured') : (zh ? '未接管' : 'Not captured')} /> : null}
      <Metric label={zh ? '国内域名规则' : 'Domestic domains'} value={String(status?.domestic_domain_count ?? 0)} />
    </div>
    {port53 && !port53.listening ? <Alert type="warning" showIcon message={zh ? 'DNS 53 端口未接管' : 'DNS port 53 was not captured'} description={port53.error || (zh ? '未提供失败原因' : 'No failure reason was reported')} /> : null}
    {status?.last_error ? <Alert type="error" showIcon message={status.last_error} /> : null}
    <div className="grid gap-3 md:grid-cols-2"><SettingSwitch title={zh ? '启用前置 DNS' : 'Enable DNS frontend'} detail={zh ? '直连规则使用直连 DNS，其余查询进入当前核心。' : 'Resolve direct rules through direct DNS and send the rest to the active core.'} checked={current.enabled} onChange={(enabled) => setDraft({ ...current, enabled })} /><SettingSwitch title={zh ? '前置层拒绝 HTTPS 记录' : 'Reject HTTPS records at frontend'} detail={zh ? '仅影响前置层，不修改当前 Profile 的核心 DNS 设置。' : 'Affects only the frontend and does not modify the active profile DNS.'} checked={current.reject_https} onChange={(reject_https) => setDraft({ ...current, reject_https })} /><SettingSwitch title={zh ? '记录 DNS 查询' : 'Record DNS queries'} checked={current.query_log_enabled} onChange={(query_log_enabled) => setDraft({ ...current, query_log_enabled })} /><label className="flex items-center gap-3 rounded-md border border-[var(--border)] p-3 text-sm"><span className="flex-1">{zh ? '最多保留条数' : 'Maximum entries'}</span><InputNumber min={100} max={20000} value={current.query_log_max_entries} onChange={(query_log_max_entries) => setDraft({ ...current, query_log_max_entries: query_log_max_entries ?? 2000 })} /></label></div>
    <DnsUpstreamsInput key={current.revision} upstreams={current.direct_upstreams} onChange={(direct_upstreams) => setDraft({ ...current, direct_upstreams })} zh={zh} />
    <div className="rounded-md border border-[var(--border)] p-3 text-xs leading-5 text-[var(--muted)]"><div>{zh ? '核心入口：' : 'Core upstream: '}{status?.core_upstream || '-'}</div><div>{zh ? '当前直连上游：' : 'Active direct upstreams: '}{status?.direct_upstreams?.join(', ') || '-'}</div><div>{zh ? '原始 DNS（只读）：' : 'Original DNS (read-only): '}{captured.join(', ') || '-'}</div><div>{zh ? '国内规则：内置 domains-min' : 'Domestic rules: built-in domains-min'}</div></div>
  </div>
}
