...
export function Logs() {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-2"><div className="relative min-w-64 flex-1"><Search className="absolute left-3 top-2.5 text-[var(--muted)]" size={16} /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('search')} /></div><select className="h-9 rounded-md border border-[var(--border)] bg-[var(--surface)] px-3 text-sm" value={level} onChange={(event) => setLevel(event.target.value)}><option value="">{t('all')}</option>{levels.map((value) => <option key={value}>{value}</option>)}</select><Button title={paused ? t('resume') : t('pause')} onClick={() => setPaused(!paused)}>{paused ? <Play size={16} /> : <Pause size={16} />}{paused ? t('resume') : t('pause')}</Button><Button size="icon" title={t('export')} onClick={exportLogs}><Download size={16} /></Button><Button size="icon" variant="danger" title={t('clear')} onClick={() => setLogs([])}><Trash2 size={16} /></Button></div>
