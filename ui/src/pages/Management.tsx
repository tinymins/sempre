...
function CorePanel() {
  ...
  return <Section title={t('core')} icon={<Package size={18} />} notice={notice}>
    <div className="grid gap-4 border-b border-[var(--border)] pb-6 md:grid-cols-[minmax(0,1fr)_auto_auto]"><Field label={t('reference')}><Input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="sing-box@stable" /></Field><Button className="self-end" variant="primary" disabled={action.isPending} onClick={() => action.mutate({ operation: 'install', value: reference })}>{action.isPending ? <Spinner /> : <Download size={16} />}{t('install')}</Button><Button className="self-end" disabled={action.isPending} onClick={() => action.mutate({ operation: 'update', value: reference })}><RefreshCw size={16} />{t('update')}</Button></div>
    <h3 className="mt-6 text-sm font-semibold">{t('installedVersions')}</h3>
    <div className="mt-3 overflow-hidden rounded-lg border border-[var(--border)]"><table className="w-full min-w-[650px] text-left text-sm"><thead className="text-xs text-[var(--muted)]"><tr><th className="px-3 py-3 font-medium">{t('core')}</th><th className="px-3 py-3 font-medium">{t('version')}</th><th className="px-3 py-3 font-medium">Channel</th><th className="px-3 py-3 font-medium">{t('details')}</th><th className="w-48" /></tr></thead><tbody>{cores.data?.installed.map((item) => { const selected = cores.data?.selected?.core === item.core && (cores.data.selected.ref === item.version || item.channels.includes(cores.data.selected.ref)); return <tr key={`${item.core}-${item.version}`} className="border-t border-[var(--border)]"><td className="px-3 py-3 font-medium">{item.core}</td><td className="px-3 py-3 font-mono text-xs">{item.version}</td><td className="px-3 py-3">{item.channels.map((channel) => <Badge key={channel}>{channel}</Badge>)}</td><td className="px-3 py-3 text-xs text-[var(--muted)]">{compactHash(item.installation.digest)} · {formatDate(item.installation.installed_at)}</td><td className="px-3 py-2 text-right"><div className="flex justify-end gap-2">{selected ? <Badge tone="success">{t('selected')}</Badge> : <Button size="small" onClick={() => action.mutate({ operation: 'use', value: `${item.core}@${item.version}` })}>{t('use')}</Button>}<Button size="icon" variant="ghost" title={t('remove')} disabled={selected} onClick={() => action.mutate({ operation: 'remove', value: `${item.core}@${item.version}` })}><Trash2 size={15} /></Button></div></td></tr> })}</tbody></table></div>
  </Section>
}
