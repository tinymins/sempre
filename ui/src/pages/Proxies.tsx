...
import { Activity, Check, ChevronDown, Gauge, RefreshCw, Search } from 'lucide-react'
...
export function Proxies() {
  ...
  return <div className="space-y-5">
    ...
    {proxies.isLoading ? <Loading /> : tab === 'groups' ? (
      groups.length ? <div className="grid gap-4 xl:grid-cols-2">{groups.map((group) => <ProxyGroup key={group.name} group={group} onSelect={(proxy) => select.mutate({ group: group.name, proxy })} onDelay={(proxy) => delay.mutate(proxy)} busy={select.isPending || delay.isPending} />)}</div> : <EmptyState title={t('noData')} detail={t('noDataDetail')} />
