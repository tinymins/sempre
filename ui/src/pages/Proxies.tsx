...
function ProxyGroup({ group, onSelect, onDelay, busy }: { group: ProxyNode; onSelect: (name: string) => void; onDelay: (name: string) => void; busy: boolean }) {
  ...
  return <Card className="overflow-hidden"><button className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-[var(--surface-hover)]" onClick={() => setOpen(!open)}><div className="min-w-0 flex-1"><h2 className="truncate text-sm font-semibold">{group.name}</h2><p className="mt-1 truncate text-xs text-[var(--muted)]">{group.type} · {group.all?.length || 0} nodes</p></div><Badge tone="success">{group.now || '-'}</Badge><ChevronDown className={`transition-transform ${open ? 'rotate-180' : ''}`} size={17} /></button>{open ? <div className="grid grid-cols-[repeat(auto-fill,minmax(15rem,1fr))] gap-2 border-t border-[var(--border)] p-3" role="radiogroup">{group.all?.map((name) => { const active = name === group.now; const selectNode = () => { if (!busy && !active) onSelect(name) }; return <div key={name} role="radio" aria-checked={active} aria-disabled={busy} tabIndex={busy ? -1 : 0} className={`flex min-w-0 cursor-pointer items-center gap-2 rounded-md border px-3 py-2.5 transition-colors ${active ? 'border-emerald-500/40 bg-emerald-500/12' : 'border-[var(--border)] hover:bg-[var(--surface-hover)]'} ${busy ? 'pointer-events-none opacity-50' : ''}`} onClick={selectNode} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectNode() } }}><span className="min-w-0 flex-1 truncate text-sm">{name}</span><Button size="icon" variant="ghost" title={t('testLatency')} disabled={busy} onClick={(event) => { event.stopPropagation(); onDelay(name) }} onKeyDown={(event) => event.stopPropagation()}>{busy ? <Spinner /> : <Gauge size={15} />}</Button></div> })}</div> : null}</Card>
}
