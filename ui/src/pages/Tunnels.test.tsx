...
describe('Tunnels', () => {
  ...
  it('hides generated IDs and keeps advanced parameters collapsed by default', async () => {
    ...
    expect(screen.queryByText('DNS 服务器')).not.toBeInTheDocument()
    expect(screen.queryByText('WSS 服务地址')).not.toBeInTheDocument()
    expect(screen.getByText('服务端口')).toBeInTheDocument()
    expect(screen.queryByText('TCP 端口')).not.toBeInTheDocument()
    ...
    expect(screen.getByLabelText('WebSocket ping')).toHaveValue('')
    expect(screen.getByLabelText('WebSocket ping')).toHaveAttribute('placeholder', '15s')
    expect(screen.getByLabelText('最大重连退避')).toHaveValue('')
    expect(screen.getByLabelText('最大重连退避')).toHaveAttribute('placeholder', '30s')
    expect(screen.getByLabelText('Upgrade path')).toHaveValue('')
    expect(screen.getByLabelText('Upgrade path')).toHaveAttribute('placeholder', 'v1')
  ...
  it('builds internal wstunnel endpoint URLs from domain and port fields', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '高级参数' }))
    const resolverInput = screen.getByLabelText('DNS 服务器')
    expect(resolverInput).toHaveAttribute('placeholder', '223.5.5.5:53')
    fireEvent.change(resolverInput, { target: { value: '2' } })
    expect(resolverInput).toHaveValue('2')
    fireEvent.change(resolverInput, { target: { value: '22' } })
    expect(resolverInput).toHaveValue('22')
    fireEvent.change(resolverInput, { target: { value: '://' } })
    expect(resolverInput).toHaveValue('://')
    expect(screen.getByRole('button', { name: '保存' })).toBeDisabled()
    fireEvent.change(resolverInput, { target: { value: '223.5.5.5:53' } })
    expect(resolverInput).toHaveValue('223.5.5.5:53')
    expect(screen.getByRole('button', { name: '保存' })).toBeEnabled()
    fireEvent.click(screen.getByRole('combobox'))
    fireEvent.click(await screen.findByText('DoT'))
    expect(resolverInput).toHaveAttribute('placeholder', 'dns.alidns.com:853')
    fireEvent.click(screen.getByRole('combobox'))
    fireEvent.click(await screen.findByText('DoH'))
    expect(resolverInput).toHaveAttribute('placeholder', 'https://dns.alidns.com/dns-query')
    fireEvent.change(resolverInput, { target: { value: 'https://dns.alidns.com/dns-query' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))

    await waitFor(() => expect(bodies).toHaveLength(1))
    const saved = bodies[0] as TunnelStatus['config']
    expect(saved.instances[0].server_url).toBe('wss://hz.example.com:443')
    expect(saved.instances[0].dns_resolvers).toEqual(['dns+https://dns.alidns.com:443/dns-query?sni=dns.alidns.com'])
  })
})
