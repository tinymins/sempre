...
import { Alert, Button, Card, Input, InputNumber, Switch, TextArea } from '@acme/components'
    ...
    {config.instances.length === 0 ? <Card className="p-8 text-center text-sm text-[var(--muted)]">{copy.empty}</Card> : config.instances.map((instance, index) => {
      ...
      return <Card key={instance.id} className="!rounded-lg" bodyStyle={{ padding: '1rem' }}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label={copy.instanceID}><Input value={instance.id} onChange={(event) => updateInstance(config, index, { id: slug(event.target.value) }, update)} /></Field>
...
function ForwardRow({ forward, copy, onChange, onRemove }: { forward: TunnelForward; copy: Record<keyof typeof en, string>; onChange: (change: Partial<TunnelForward>) => void; onRemove: () => void }) {
  return <div className="grid items-end gap-2 rounded-md border border-[var(--border)] p-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_120px_1fr_120px_110px_auto]">
    <Field label={copy.name}><Input value={forward.name} onChange={(event) => onChange({ name: event.target.value })} /></Field><Field label="Forward ID"><Input value={forward.id} onChange={(event) => onChange({ id: slug(event.target.value) })} /></Field><Field label={copy.localPort}><InputNumber className="w-full" min={1} max={65535} value={forward.listen_port} onChange={(value) => onChange({ listen_port: value ?? 0 })} /></Field><Field label={copy.remoteHost}><Input value={forward.remote_host} onChange={(event) => onChange({ remote_host: event.target.value })} /></Field><Field label={copy.remotePort}><InputNumber className="w-full" min={1} max={65535} value={forward.remote_port} onChange={(value) => onChange({ remote_port: value ?? 0 })} /></Field><Field label={copy.timeout}><InputNumber className="w-full" min={0} value={forward.timeout_seconds} onChange={(value) => onChange({ timeout_seconds: value ?? 0 })} /></Field><Button size="small" variant="danger" icon={<Trash2 size={14} />} onClick={onRemove}>{copy.remove}</Button>
  </div>
}
...
function slug(value: string) { return value.toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+/, '').slice(0, 63) }
