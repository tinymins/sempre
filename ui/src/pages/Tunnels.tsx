...
export function Tunnels() {
  ...
  return <div className="space-y-5">
    ...
    {config.instances.length === 0 ? <Card className="p-8 text-center text-sm text-[var(--muted)]">{copy.empty}</Card> : config.instances.map((instance, index) => {
      ...
      const resolverPort = resolver.host ? resolver.port : resolverDefaultPort(resolverType)
      return <Card key={instance.id} className="!rounded-lg" bodyStyle={{ padding: '1rem' }}>
        ...
        <div className="mt-4 grid items-end gap-3 md:grid-cols-2 xl:grid-cols-[1fr_2fr_120px_auto]">
          ...
          <Field label={copy.serverPort}><InputNumber className="w-full" min={1} max={65535} value={server.port} onChange={(value) => updateInstance(config, index, { server_url: endpointURL('wss', server.host, value ?? 443) }, update)} /></Field>
        ...
        <Collapse className="mt-4" size="small" items={[{ key: 'advanced', label: copy.advancedSettings, children: <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[2fr_1fr_1fr_1fr]">
          <div className="grid gap-2 sm:grid-cols-[100px_minmax(0,1fr)_90px]"><Field label={copy.resolverType}><Select value={resolverType} options={resolverTypeOptions} onChange={(value) => { const nextType = value as ResolverType; setResolverDraftTypes((current) => ({ ...current, [instance.id]: nextType })); if (resolver.host) updateInstance(config, index, { dns_resolvers: resolverList(nextType, resolver.host, resolverDefaultPort(nextType), resolver.sni) }, update) }} /></Field><Field label={copy.resolverHost}><Input value={resolver.host} placeholder={copy.resolverPlaceholder} onChange={(event) => updateInstance(config, index, { dns_resolvers: resolverList(resolverType, event.target.value, resolverPort, resolver.sni) }, update)} /></Field><Field label={copy.resolverPort}><InputNumber className="w-full" min={1} max={65535} value={resolverPort} onChange={(value) => updateInstance(config, index, { dns_resolvers: resolverList(resolverType, resolver.host, value ?? resolverDefaultPort(resolverType), resolver.sni) }, update)} /></Field>{resolverType === 'dns' ? null : <div className="sm:col-span-3"><Field label={copy.resolverSNI}><Input value={resolver.sni} placeholder="dns.example.com" onChange={(event) => updateInstance(config, index, { dns_resolvers: resolverList(resolverType, resolver.host, resolverPort, event.target.value) }, update)} /></Field></div>}</div>
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3"><Field label="Upgrade path"><Input value={instance.upgrade_path_prefix || ''} placeholder="v1" onChange={(event) => updateInstance(config, index, { upgrade_path_prefix: event.target.value }, update)} /></Field><Field label={copy.preferIPv4}><Switch checked={instance.prefer_ipv4} onChange={(checked) => updateInstance(config, index, { prefer_ipv4: checked }, update)} /></Field></div>
        </div> }]} />
...
function resolverDefaultPort(type: ResolverType) { return type === 'dns' ? 53 : type === 'doh' ? 443 : 853 }
function parseResolver(value: string): { type: ResolverType; host: string; port: number; sni: string } { try { const parsed = new URL(value); const type = parsed.protocol === 'dns+https:' ? 'doh' : parsed.protocol === 'dns+tls:' ? 'dot' : 'dns'; return { type, host: parsed.hostname.replace(/^\[|\]$/g, ''), port: Number(parsed.port) || resolverDefaultPort(type), sni: parsed.searchParams.get('sni') || '' } } catch { return { type: 'dns', host: '', port: 53, sni: '' } } }
function resolverList(type: ResolverType, host: string, port: number, sni: string) { const scheme = type === 'dns' ? 'dns' : type === 'doh' ? 'dns+https' : 'dns+tls'; const base = endpointURL(scheme, host, port); const value = base && type !== 'dns' && sni.trim() ? `${base}?sni=${encodeURIComponent(sni.trim())}` : base; return value ? [value] : [] }
