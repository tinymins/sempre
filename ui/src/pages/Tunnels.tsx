...
export function Tunnels() {
  ...
  return <div className="space-y-5">
    ...
    {config.instances.length === 0 ? <Card className="p-8 text-center text-sm text-[var(--muted)]">{copy.empty}</Card> : config.instances.map((instance, index) => {
      ...
      const resolverInput = formatResolverInput(resolver, resolverType)
      return <Card key={instance.id} className="!rounded-lg" bodyStyle={{ padding: '1rem' }}>
        ...
        <Collapse className="mt-4" size="small" items={[{ key: 'advanced', label: copy.advancedSettings, children: <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[2fr_1fr_1fr_1fr]">
          <div className="grid gap-2 sm:grid-cols-[100px_minmax(0,1fr)]"><Field label={copy.resolverType}><Select value={resolverType} options={resolverTypeOptions} onChange={(value) => { const nextType = value as ResolverType; setResolverDraftTypes((current) => ({ ...current, [instance.id]: nextType })); if (resolver.host) updateInstance(config, index, { dns_resolvers: resolverList(nextType, formatResolverInput(resolver, nextType)) }, update) }} /></Field><Field label={copy.resolverServer}><Input value={resolverInput} placeholder={resolverPlaceholder(resolverType)} onChange={(event) => updateInstance(config, index, { dns_resolvers: resolverList(resolverType, event.target.value) }, update)} /></Field></div>
...
type ParsedResolver = { type: ResolverType; host: string; port: number; path: string; search: string }
function parseResolver(value: string): ParsedResolver { try { const parsed = new URL(value); const type = parsed.protocol === 'dns+https:' ? 'doh' : parsed.protocol === 'dns+tls:' ? 'dot' : 'dns'; return { type, host: parsed.hostname.replace(/^\[|\]$/g, ''), port: Number(parsed.port) || resolverDefaultPort(type), path: parsed.pathname === '/' ? '' : parsed.pathname, search: parsed.search } } catch { return { type: 'dns', host: '', port: 53, path: '', search: '' } } }
function resolverPlaceholder(type: ResolverType) { return type === 'dns' ? '223.5.5.5:53' : type === 'doh' ? 'https://dns.alidns.com/dns-query' : 'dns.alidns.com:853' }
function formatHostPort(host: string, port: number) { const clean = host.replace(/^\[|\]$/g, ''); return clean ? `${clean.includes(':') ? `[${clean}]` : clean}:${port}` : '' }
function formatResolverInput(resolver: ParsedResolver, type: ResolverType) {
  if (!resolver.host) return ''
  const port = type === resolver.type ? resolver.port : resolverDefaultPort(type)
  const authority = formatHostPort(resolver.host, port)
  if (type === 'dns') return authority
  const params = new URLSearchParams(type === resolver.type ? resolver.search : '')
  if (params.get('sni') === resolver.host) params.delete('sni')
  const search = params.size ? `?${params.toString()}` : ''
  if (type === 'doh') return `https://${authority}${resolver.type === 'doh' && resolver.path ? resolver.path : '/dns-query'}${search}`
  return `${authority}${search}`
}
function isIPAddress(host: string) { return host.includes(':') || /^\d{1,3}(?:\.\d{1,3}){3}$/.test(host) }
function resolverList(type: ResolverType, input: string) {
  const value = input.trim()
  if (!value) return []
  try {
    const parsed = new URL(type === 'doh' ? (/^https?:\/\//i.test(value) ? value : `https://${value}`) : `${type === 'dns' ? 'dns' : 'tls'}://${value}`)
    const host = parsed.hostname.replace(/^\[|\]$/g, '')
    const port = Number(parsed.port) || resolverDefaultPort(type)
    const scheme = type === 'dns' ? 'dns' : type === 'doh' ? 'dns+https' : 'dns+tls'
    const params = new URLSearchParams(parsed.search)
    if (type !== 'dns' && !params.has('sni') && !isIPAddress(host)) params.set('sni', host)
    const path = type === 'doh' && parsed.pathname !== '/' ? parsed.pathname : ''
    const search = params.size ? `?${params.toString()}` : ''
    return [`${endpointURL(scheme, host, port)}${path}${search}`]
  } catch { return [] }
}
